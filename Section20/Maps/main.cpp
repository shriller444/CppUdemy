...
// prints the first and second value of the map in a nice format for string and sets nicely 
...
// displays maps of various types 
...
void test1() {
    ...
    m.insert(std::pair<std::string, int>("Anna", 10)); // insert this value , sorts automatically when adding. 
    display(m);
    
    m.insert(std::make_pair("Joe", 5));
    display(m);
    
    m["Frank"] = 18; // create frank and set his kill count to 18.
    display(m);
    
    m["Frank"] += 10; // increment franks value by 10 to make 28.         
    display(m);
    
    m.erase("Frank"); // remove frank 
    display(m);
    ...
    if (it != m.end())
        std::cout << "Found: " << it->first << " : " << it->second << std::endl;    // display larry name and age if found
        
    m.clear(); // clear all map values. 
