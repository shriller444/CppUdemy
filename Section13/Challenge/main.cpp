...
-main.clTabCtrlcpp - the main driver that creates a Movies object and adds and increments movies
...
void addMovie(Movies &movie, std::string movieName, std::string rating, int watchedVal);
void incrementWatched(Movies &movie, std::string movieName);

using namespace std;

void addMovie(Movies &movie, std::string movieName, std::string rating, int watchedVal){
    if (movie.addMovie(movieName, rating, watchedVal)){
        std::cout << movieName << " added " << endl;
    }
    else{
        std::cout << movieName << " not added " << std::endl;
    }
}

void incrementWatched(Movies &movie, std::string movieName){
    if (movie.incrementWatched(movieName)){
        std::cout << "Incremented watch count for " << movieName << std::endl;
    }
    else{
        std::cout << "That movie " << movieName << " does not exist in your movie list" << endl;
    }
}
...
int main(){
    ...
    addMovie(myMovieList, "CBT", "PG-13", 1);
    myMovieList.display();
    incrementWatched(myMovieList, "CBT");
    myMovieList.display();
